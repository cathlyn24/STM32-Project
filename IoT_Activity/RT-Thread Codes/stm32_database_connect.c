...
/* CLOUD CONFIGURATION */
#define API_URL "http://cathlynramo.pythonanywhere.com/api/upload"
...
static void upload_data(float ax, float ay, float az, float gx, float gy, float gz, char *label)
{
    ...
    // Convert raw values to g-units (ICM20608: ±2g range = 16384 LSB/g)
    float ax_g = ax / 16384.0f;
    float ay_g = ay / 16384.0f;
    float az_g = az / 16384.0f;

    // Convert gyro to degrees/sec (ICM20608: ±250°/s range = 131 LSB/°/s)
    float gx_dps = gx / 131.0f;
    float gy_dps = gy / 131.0f;
    float gz_dps = gz / 131.0f;

    // Create JSON with converted values
    cJSON *root = cJSON_CreateObject();
    cJSON_AddNumberToObject(root, "ax", (double)ax_g);
    cJSON_AddNumberToObject(root, "ay", (double)ay_g);
    cJSON_AddNumberToObject(root, "az", (double)az_g);
    ...
    // Build HTTP POST request - FIX: Use /api/upload NOT /api/data
    ...
    if (received > 0) {
        ...
        } else if (rt_strstr(response, "404") != RT_NULL) {
            rt_kprintf("✗✗✗ FAILURE: 404 - Wrong endpoint! ✗✗✗\n");
        } else {
